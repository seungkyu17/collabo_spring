...
public class ProductController {
    ...
    //@PathVariable 은 URL의 경로 변수를 메소드의 매개 변수로 값을 전달해 줍니다.
    ...
    public ResponseEntity<?> insert(@RequestBody Product product){
        ...
        String imageData = product.getImage(); //'Base64' 인코딩 문자열(엄청 길다.)
        ...
        try{
            ...
            return ResponseEntity.status(500).body(Map.of("message", err.getMessage(), "image", "Error file uploading"));
        }
    }

    //프론트엔드의 상품 수정 페이지에서 요청이 들어왔습니다.
    @GetMapping("/update/{id}") //상품의 'id 정보'를 이용하여 해당 상품 Bean 객체를 반환해 줍니다.
    public ResponseEntity<Product> getUpdate(@PathVariable Long id){
        System.out.println("수정할 상품 번호 : " + id);
        
        Product product = this.productService.getProductById(id);
        
        if(product == null){ //상품이 없으면 404 응답과 함께 null 을 반환
...
//        System.out.println(updatedProduct);
        ...
        if(findProduct.isEmpty()){ //상품이 존재하지 않으면 404 응답 반환
            ...
            //Optional 에서 실제 상품 정보 끄집어내기.
            Product savedProduct = findProduct.get();

            try{
                //이전 이미지 객체에 새로운 이미지 객체 정보를 업데이트 합니다.
    ...
    private String saveProductImage(String base64Image) {
        ...
        //base64Image : JavaScript FileReader API 에 만들어진 이미지입니다.
        byte[] decodedImage = Base64.getDecoder().decode(base64Image.split(",")[1]);

        try{ //FileOutputStream 은 바이트 파일을 처리해주는 자바의 'Stream' 클래스이다.
            FileOutputStream fos = new FileOutputStream(imageFile);
