...
public class MemberController {
    ...
    public ResponseEntity<?> signup(@Valid @RequestBody Member bean, BindingResult bindingResult){ //회원 가입을 위한 컨트롤러 메소드
        //ResponseEntity : HTTP 응답 코드(숫자형식)나, 적절한 메시지 등을 표현하기 위한 클래스
        ...
        //여기서, Field 는 각각의 변수를 의미합니다.
        System.out.println(bindingResult.getFieldErrorCount());

        if(bindingResult.hasErrors()) { //유효성 검사에 문제가 있음.
            // Map<컬럼 이름, 오류 메시지> 형식으로 만들어서 클라이언트에게 전송해 줍니다.
            Map<String, String> errors = new HashMap();

            for(FieldError err:bindingResult.getFieldErrors()) {
                //errors.put(컬럼 이름, 오류 메시지)
...
//        System.out.println(bean);
//
//        //member 는 데이터 베이스에서 내가 조회한 회원 정보를 담고 있는 객체
...
//        }else{
//            response.put("message", "id 또는 비밀번호가 잘못 되었습니다.");
//            //응답 코드 '401'은 인증 실패(Unauthorized)를 의미합니다.
